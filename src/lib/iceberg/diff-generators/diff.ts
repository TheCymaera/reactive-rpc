export namespace diff {
	...
	export const EncodedMimeType = "application/json"; //"application/x.iceberg.diff";
	
	export const encode = encodeDiffs;
	export const decode = decodeDiffs;

	export const empty = () => encodeDiffs([]);
}
...
export function encodeDiffs(diffs: diff.Diffs): string {
	return JSON.stringify(diffs);
}

export function decodeDiffs(encoded: string): diff.Diffs {
	if (!encoded) return [];
	return JSON.parse(encoded) as diff.Diffs;
}
